...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split on sentence boundaries retaining punctuation using positive lookbehind
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        # Base case: no more separators to try, split by raw character chunk_size
        ...
        # If current separator did not divide the text, fall back to next separator
        if len(splits) == 1:
            return self._split(current_text, next_separators)

        # 1. Split down: recursively process pieces that still exceed chunk_size
        pieces: list[str] = []
        for piece in splits:
            if not piece:
                continue
        ...
        # 2. Merge up: combine adjacent small pieces as long as they fit within chunk_size
        merged: list[str] = []
        current_merged = ""
        for piece in pieces:
            if not current_merged:
                current_merged = piece
            else:
                candidate = current_merged + (sep if sep != "" else "") + piece
...
class HeadingChunker:
    ...
    If a section exceeds chunk_size, it splits using RecursiveChunker and prepends the heading
    title to child chunks so semantic context is never lost.
    """
        ...
        for section in sections:
            ...
            if len(section) <= self.chunk_size:
                ...
                for sc in sub_chunks:
                    if heading and not sc.startswith('#'):
